let formularios = {}; // Objeto para almacenar los formularios creados
let formularioDatos = {};
...
Si no hay claridad sería bueno que se comunicaran con nosotros para aclarar</p>
    ...
    `;
        ...
        case 'crear-formulario':
            content.innerHTML = `
        <form id="crearFormularioForm" class="register-form">
            <h2 id="focus-group-header">Encabezados Encuesta</h2>
            
            <!-- Nombre del formulario -->
            <div class="form-group">
                <label for="nombreFormulario">Nombre del Formulario:</label>
                <input type="text" id="nombreFormulario" required>
            </div>
            
            <!-- Contenedor de encabezados -->
            <div id="headersContainer" class="form-group">
                <h3>Encabezados</h3>
            </div>
            
            <!-- Botón para agregar encabezado -->
            <div class="form-group">
                <button type="button" id="addHeaderBtn" class="filter-box button">Agregar Encabezado</button>
                <div class="filter-boxdos">
                    <button type="button" id="saveDraftBtn" class="filter-box button">Guardado Preliminar</button>
                    <button type="button" id="clearFormBtn" class="filter-box button">Limpiar</button>
                </div>
            </div>

            <!-- Contenedor de lista de encabezados preliminares -->
            <div id="preliminaryHeadersContainer" class="form-group"></div>
            
            <!-- Botón confirmar encuesta -->
            <div class="form-group">
                <button type="button" id="confirmSurveyBtn" class="filter-box button">CONFIRMAR ENCUESTA</button>
            </div>
        </form>
    `;
            ...
            const preliminaryHeadersContainer = document.getElementById("preliminaryHeadersContainer");
            const confirmSurveyBtn = document.getElementById("confirmSurveyBtn");

            let preliminaryHeaders = [];

            // Agregar encabezados
            addHeaderBtn.addEventListener("click", () => {
                const headerDiv = document.createElement("div");
                headerDiv.className = "form-group vertical-header";
                headerDiv.innerHTML = `
            <label>Encabezado:</label>
            <input type="text" class="header-input" required>
            <button type="button" class="filter-box button remove-header-btn">Eliminar</button>
        `;
            ...
            saveDraftBtn.addEventListener("click", () => {
                const headers = Array.from(headersContainer.querySelectorAll(".header-input"))
                    .map(input => input.value.trim())
                    .filter(value => value);

                preliminaryHeaders = [...headers];
                displayPreliminaryHeaders();
            });

            function displayPreliminaryHeaders() {
                preliminaryHeadersContainer.innerHTML = `
            <h3>Encabezados Guardados:</h3>
            <ul id="headersList">
                ${preliminaryHeaders
                    .map((header, index) => `
                        <li class="header-item">
                            <input type="text" value="${header}" data-index="${index}" class="header-edit">
                            <div class="button-container">
                                <button type="button" class="filter-box button edit-header-btn" data-index="${index}">Editar</button>
                                <button type="button" class="filter-box button delete-header-btn" data-index="${index}">Eliminar</button>
                            </div>
                        </li>
                    `).join("")}
            </ul>
        `;
                ...
                preliminaryHeadersContainer.querySelectorAll(".edit-header-btn").forEach(button => {
                    button.addEventListener("click", event => {
                        const index = event.target.dataset.index;
                        const newValue = preliminaryHeadersContainer.querySelector(`.header-edit[data-index="${index}"]`).value.trim();
                        if (newValue) {
                            preliminaryHeaders[index] = newValue;
                        }
                        displayPreliminaryHeaders();
                    });
                });

                preliminaryHeadersContainer.querySelectorAll(".delete-header-btn").forEach(button => {
                    button.addEventListener("click", event => {
                        const index = event.target.dataset.index;
                        preliminaryHeaders.splice(index, 1);
                        displayPreliminaryHeaders();
                    });
                });
            }
            ...
            // Confirmar encuesta
            confirmSurveyBtn.addEventListener("click", () => {
                const nombreFormulario = document.getElementById("nombreFormulario").value.trim();
                if (!nombreFormulario || preliminaryHeaders.length === 0) {
                    alert("Debe completar todos los campos antes de confirmar.");
                    return;
                }

                formularios[nombreFormulario] = preliminaryHeaders;
                alert(`Encuesta "${nombreFormulario}" confirmada.`);
                window.location.hash = "listar-formularios";
            });
            ...
            break;
        case 'listar-formularios':
            content.innerHTML = `
                        <h2 id="focus-group-header">Consultar y Registrar Encuesta</h2>
                        <div class="form-group">
                            <label for="formSelect">Seleccionar Formulario:</label>
                            <select id="formSelect" class="filter-box select">
                                <option value="" disabled selected>Seleccione un formulario</option>
                            </select>
                        </div>
                        <div id="formDataContainer" class="filter-box"></div>
            ...
            Object.keys(formularios).forEach(formName => {
                const option = document.createElement("option");
            ...
            formSelect.addEventListener("change", () => {
                ...
                if (formularios[selectedForm]) {
                    const headers = formularios[selectedForm];
                    formDataContainer.innerHTML = `
                                <form id="dataEntryForm" class="register-form2">
                                    <h3 id="titulo_h1">Formulario: ${selectedForm}</h3>
                                    ${headers
                                        .map(header => `
                                            <div class="form-group">
                                                <label>${header}: <input type="text" data-header="${header}" required></label>
                                            </div>
                                        `)
                                        .join("")}
                                    <button type="submit" class="filter-box button">Guardar Datos</button>
                                </form>
                            `;
                    ...
                    dataEntryForm.addEventListener("submit", (event) => {
                        ...
                        headers.forEach(header => {
                            const input = dataEntryForm.querySelector(`input[data-header="${header}"]`);
                            formData[header] = input.value;
                        });

                        if (!formularioDatos[selectedForm]) {
                            formularioDatos[selectedForm] = [];
                        }
                        formularioDatos[selectedForm].push(formData);

                        updateDataTable(selectedForm);
                        dataEntryForm.reset();
            ...
            function updateDataTable(formName) {
                const data = formularioDatos[formName] || [];
                const headers = formularios[formName] || [];

                if (data.length === 0) {
                    dataTableContainer.innerHTML = `<p>No hay datos para mostrar.</p>`;
                    return;
                }
